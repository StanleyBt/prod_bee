...
if __name__ == "__main__":
    ...
    if not initialize_mem0_service():
        print("Failed to initialize Mem0 service")
        exit(1)

    # Use a test key
    test_key = "test_tenant:test_user:test_module:test_session"
    # Hardcoded test messages
    messages = [
        {"role": "user", "content": "test user"},
        {"role": "assistant", "content": "test assistant"}
    ]
    print(f"Storing test memory for user_id={test_key} ...")
    result = mem0_client.add(
        user_id=test_key,
        messages=messages
    )
    print("Result from mem0_client.add:", result) 