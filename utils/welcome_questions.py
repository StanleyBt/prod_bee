...
def get_questions_by_module(
    ...
    """
    Get questions for a specific module and role.
    ...
    Returns:
        List of questions for the module and role
    """
    questions = get_welcome_questions_by_role(tenant_id, role)
    
    if module in questions:
        # For HR role, return all questions for the module
        if role.lower() == "hr":
            all_module_questions = []
            for role_questions in questions[module].values():
                all_module_questions.extend(role_questions)
            return all_module_questions
        # For other roles, return only their questions
        elif role.lower() in questions[module]:
            return questions[module][role.lower()]
    
    return []

# Frontend helper functions (for reference)
def get_frontend_filtering_example():
    """
    Example of how frontend should filter questions based on user role.
    This is for reference - actual implementation will be in frontend.
    """
    return """
    # Frontend JavaScript Example:
    
    function filterQuestionsByRole(allQuestions, userRole) {
        if (userRole === 'hr') {
            return allQuestions; // HR sees everything
        }
        
        // Other roles only see their own questions
        const filteredQuestions = {};
        for (const [module, moduleQuestions] of Object.entries(allQuestions)) {
            if (moduleQuestions[userRole]) {
                filteredQuestions[module] = {
                    [userRole]: moduleQuestions[userRole]
                };
            }
        }
    ...
    // Usage:
    const userRole = 'employee'; // From user session/authentication
    const filteredQuestions = filterQuestionsByRole(response.questions, userRole);
    """ 